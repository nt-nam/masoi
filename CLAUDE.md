...
| `docs/PROGRESS.md` | Theo dõi task atomic theo format mô tả trong PLAN — cập nhật mỗi đợt làm việc. |
...
pnpm dev                              # server :3210 + web :5173 (concurrently, hot-reload)
...
pnpm --filter @masoi/server start     # production local: serve luôn apps/web/dist trên :3210
```

- `MASOI_TIMER_SCALE=0.25` (env) co mọi đồng hồ pha — dùng khi thử nghiệm/test (smoke test đặt 0.012).
- Port mặc định 3210 (3001 hay bị app khác trên máy user chiếm).
...
- KHÔNG sửa file UTF-8 tiếng Việt bằng PowerShell `Get-Content`/`Set-Content` (vỡ encoding) — dùng Edit/Write tool.
