// Chuyển LUAT_MA_SOI.md -> HTML (rồi Chrome headless in ra PDF).
// Trình chuyển Markdown tối giản, đủ cho các cấu trúc dùng trong file:
// headers, hr, blockquote, bảng GFM, list (ordered/unordered/nested),
// bold, inline code, code fence, link, emoji.
import { readFileSync, writeFileSync } from "node:fs";

const src = readFileSync("LUAT_MA_SOI.md", "utf8");
const lines = src.replace(/\r\n/g, "\n").split("\n");

const esc = (s) =>
  s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");

// inline: code -> bold -> link (escape phần text thường)
function inline(text) {
  const parts = [];
  let i = 0;
  const codeRe = /`([^`]+)`/g;
  let last = 0, m;
  // tách inline code trước để không format bên trong
  const segs = [];
...
while (i < lines.length) {
  ...
  // blockquote (gộp các dòng > liên tiếp)
  if (/^>\s?/.test(line)) {
    const buf = [];
  ...
  // table (dòng hiện tại có | và dòng kế là separator)
  ...
  // list (ordered/unordered, hỗ trợ 1 cấp lồng)
  if (/^(\s*)([-*]|\d+\.)\s+/.test(line)) {
    const items = [];
    ...
    while (i < lines.length && /^(\s*)([-*]|\d+\.)\s+/.test(lines[i])) {
      ...
      items.push({ indent, text: mm[3] });
      i++;
    }
    // render với 1 cấp lồng
    let html = `<${listType}>`;
    let inSub = false;
    for (let k = 0; k < items.length; k++) {
      const it = items[k];
      const next = items[k + 1];
      if (it.indent > 0 && !inSub) {
        html += `<ul><li>${inline(it.text)}</li>`;
        inSub = true;
      } else if (it.indent > 0 && inSub) {
        html += `<li>${inline(it.text)}</li>`;
      } else {
        if (inSub) { html += "</ul>"; inSub = false; }
        html += `<li>${inline(it.text)}`;
        if (next && next.indent > 0) {
          // mở li, để sub gắn vào trong (đơn giản: đóng sau)
          html += "";
        }
        html += "</li>";
      }
...
body {
  font-family: "Segoe UI", "Noto Sans", "Noto Color Emoji", Arial, sans-serif;
  font-size: 12px; line-height: 1.6; color: #1f2328;
...
blockquote {
  margin: 12px 0; padding: 8px 14px; border-left: 4px solid #a78bfa;
  background: #f5f3ff; color: #4c1d95; border-radius: 0 6px 6px 0;
}
code {
  background: #f3f4f6; padding: 1.5px 5px; border-radius: 4px;
  font-family: "Consolas", "Courier New", monospace; font-size: 0.9em; color: #be185d;
}
pre {
  background: #1f2937; color: #e5e7eb; padding: 12px 14px; border-radius: 8px;
...
<title>Luật Game Ma Sói</title>
<style>${css}</style></head>
<body>${out.join("\n")}</body></html>`;

writeFileSync("LUAT_MA_SOI.html", html, "utf8");
console.log("HTML written: LUAT_MA_SOI.html (" + html.length + " bytes)");
